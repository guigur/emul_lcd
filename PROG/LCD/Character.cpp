...
void Character::init()
{
	...
	_height = 8;
	setPixSize(10, 10);
	_colorBackground = sf::Color::Transparent;
	_colorLetter = sf::Color::White;
	//2eme colonne
...
void Character::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (unsigned int i = 0; i < _height; i++)
	{
		for (unsigned int j = 0; j < _width; j++)
		{
			...
			pix.setPosition(abs((int)j - (int)_width + 1) * (float)_sizePixX + currentPosition.x, i * (float)_sizePixY + currentPosition.y); //flip the pixel order
			target.draw(pix);
		}
	}
